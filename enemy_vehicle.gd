...
## УРОВЕНЬ ЭТОЙ МАШИНЫ (1..N по ступеням enemy_spawner.preset_tiers). Ставит спавнер при
## рождении; нужен, чтобы метка над врагом показывала силу ДО боя — иначе разброс уровней
## («игрок третьего встречает и первый, и третий») читается как случайная несправедливость.
