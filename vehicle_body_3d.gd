...
## Собственный запас машины, БЕЗ аккумуляторов. Раньше ёмкость равнялась ровно
## batteries × BATTERY_CAP, то есть у машины без батареи была НУЛЁМ: выработка панели
## сгорала в тот же тик, а энергию можно было потратить только из выработки ТЕКУЩЕГО кадра
## (0.1 при SOLAR_RATE 6). Реген просит 2.0 за блок разом — и не чинил НИКОГДА, пока игрок
## не находил аккумулятор в другой ветке квестов. Панель и реген выдаются одной веткой и
## обязаны работать вместе; аккумулятор остаётся апгрейдом (вчетверо больше буфера).
const BASE_ENERGY_CAP := 25.0
...
func _energy_tick(delta: float) -> void:
	...
	if _cap_timer <= 0.0:
		...
		_energy_cap = BASE_ENERGY_CAP + batteries * BATTERY_CAP
