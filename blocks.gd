...
# РАЗЪЁМ блока — грань В ЕГО СОБСТВЕННОЙ, НЕПОВЁРНУТОЙ системе координат: та сторона,
# которой блок прижимается к соседу. Ставя блок, постройка САМА разворачивает его так,
# чтобы этот разъём смотрел на соседа (см. _face_orient), поэтому список ограничивает не
# «куда можно поставить», а только «чем блок крепится».
#
# Раньше здесь стояли стороны СОСЕДА («колесо цепляется слева и справа»), из-за чего колесо
# нельзя было поставить ни сверху, ни спереди, а бур — никуда, кроме морды. Имена приведены
# к реальной геометрии моделей: у колеса крепление сидит на грани +Z (в wheel.tscn модуль
# стоит в (0,0,0.5) и колесо торчит дальше по +Z) — это грань "back", как и у бура.
func _init_contacts() -> void:
	_contact_faces = {
		G.Block.GUN:    ["bottom"],          # пушка стоит на подошве
		G.Block.ROCKET: ["bottom"],
		G.Block.DRILL:  ["back"],            # бур крепится задом, коронка смотрит вперёд
		G.Block.SMALL_DRILL: ["back"],
		G.Block.WHEEL:       ["back"],       # ступица на грани +Z, колесо свисает наружу
		G.Block.SMALL_WHEEL: ["back"],
		G.Block.BIG_WHEEL:   ["back"],
		G.Block.STAB_WHEEL:  ["right"],      # у этого модель развёрнута: корпус сидит на +X
		G.Block.TOP_WHEEL:   ["back"],
		# остальные типы — все грани (по умолчанию), крепятся подошвой
	}
...
## Грань, которой блок прижимается к соседу, — ТОЛЬКО если она прописана выше. Пусто
## означает «у блока нет выделенной стороны»: такой блок встаёт РОВНО, как фабричные, и
## цепляется любой стороной. Раньше здесь по умолчанию возвращалась подошва, и обычный
## блок, поставленный на боковую грань, заваливался набок вместе со всем, что на нём
## нарисовано. Нужен блоку разворот — впиши ему грань в _init_contacts.
func primary_face(block_type: int) -> String:
	var f: Array = _contact_faces.get(block_type, [])
	return String(f[0]) if not f.is_empty() else ""
...
func can_attach(neighbor_type: int, new_type: int, attach_face: String) -> bool:
	...
	if is_station and not _allowed_on_station(new_type):
		return false
	# ГРАНЬ БОЛЬШЕ НЕ ЗАПРЕЩАЕТ. Проверка сравнивала имена граней с НЕПОВЁРНУТЫМИ разъёмами
	# обеих сторон и потому запрещала то, что прекрасно собирается: у бура разъём сзади —
	# но развернув его на 90°, он спокойно встаёт на боковую или верхнюю грань соседа; колёса
	# из-за той же проверки лезли только влево-вправо. Разъём у блока есть всегда, а ставя
	# блок, постройка сама доворачивает его этим разъёмом к соседу (_face_orient) — значит
	# «грани контакта соприкасаются» выполняется на любой грани, и запрещать нечего.
	# Бесполезная сборка (колесо на крыше) — это выбор игрока, а не ошибка.
	return true
