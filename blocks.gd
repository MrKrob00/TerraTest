...
## Грань, которой блок прижимается к соседу. У блока «со всеми гранями» это подошва —
## так исторически ориентируются обычные блоки, и менять это незачем.
func primary_face(block_type: int) -> String:
	var f: Array = _contact_faces.get(block_type, [])
	return String(f[0]) if not f.is_empty() else "bottom"
