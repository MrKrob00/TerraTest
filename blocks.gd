...
func _layout_enemy(preset: int) -> void:
	...
	# ШИРИНА КОРПУСА В КЛЕТКАХ — нечётная, кабина по центру. Раньше был флаг `wide`, то есть ровно
	# два варианта, и верхние ступени упирались в ту же тройку, что и средние.
	#
	# ПОТОЛОК СТАВЯТ КОЛЁСА, А НЕ КОРПУС: они встают на half+1 от оси, поэтому корпус в девять
	# клеток (half = 4) даёт колёса на x = 0 и x = 10, то есть впритык по сетке, а одиннадцать
	# клеток корпуса выносит их за край, и set_block молча их теряет. Замерено на движке:
	# 9 → габарит 11, 136 клеток, 12 колёс, ни одного предупреждения; 11 → 150 клеток и НОЛЬ колёс.
	#
	# Сами таблицы (ENEMY_BUILDS) пока просят 1, 3 или 5: здесь потолок сетки, а не вкуса.
	var half: int = clampi(int(b.get("width", 1)) / 2, 0, 4)
	...
	#
	# РАБОЧИЙ БЛОК ЗАНИМАЕТ ВЕСЬ НОС, а не одну клетку по центру. На корпусе в три клетки один бур
	# посередине оставлял два пустых угла, и машина размером с боевую грызла жилу ровно так же, как
	# самая мелкая: ширина ничего не давала. Каждая клетка носа стоит перед своей клеткой корпуса
	# (пол идёт от 5-half до 5+half), так что присоединяющая грань у всех одна и та же — задняя,
	# как у центрального.
