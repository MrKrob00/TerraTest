...
# КАРТА МИРА ПРЯМО В ИНСПЕКТОРЕ: меняешь сид — видишь, где что лежит.
#
...
# подряд.
#
# ЧТО ИМЕННО НАРИСОВАНО — МАСКИ БИОМОВ, А НЕ ВЫСОТЫ, и это не экономия, а выбор. Замерено:
# высота в точке стоит 124 мкс (LiteTerrainGen.height_at), три маски — около пяти. На 128×128 это
# две секунды против восьмидесяти миллисекунд, то есть «жду» против «листаю».
#
# А выбирают сид ИМЕННО ПО РАСКЛАДКЕ РЕГИОНОВ: где луг, где пустыня, где каньон и где горы.
# Рельеф внутри региона от сида к сиду отличается куда меньше, чем их расположение, и смотреть
# его надо всё равно в 3D — для этого рядом кнопка «Показать в сцене», которая строит настоящую
# землю тем же генератором.
#
...
func _redraw() -> void:
	...
	# превью и игра кладут регионы по-разному — ровно та ошибка, ради которой превью и заводят.
