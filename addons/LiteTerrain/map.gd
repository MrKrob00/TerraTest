...
## ЗА СКОЛЬКО КЛЕТОК ДО КРАЯ НАЧИНАЕМ ДВИГАТЬ ОКНО. Это ЗАПАС ХОДА, а не косметика: пока полоса
## считается, игрок продолжает ехать, и если он успевает добраться до края раньше — он сваливается
## с обрыва в пустоту, где нет ни меша, ни коллизии. Именно так «бесконечный мир» и кончался.
##
## Считать надо против скорости: 192 клетки на 20 м/с — это десять секунд, а полоса столько же и
## считается. 640 даёт больше полуминуты хода против пяти секунд счёта.
##
## Верхнюю границу всё равно держит кламп ниже: запас больше (окно − шаг) / 2 кидает точку в
## запретную зону с другой стороны сразу после сдвига, и окно начинает ездить туда-сюда каждый кадр.
@export var window_margin: int = 640
...
## КАРТА УХОДИТ ИЗ ДЕРЕВА — ОКНО БОЛЬШЕ НЕ СЧИТАЕТСЯ. Сюда приходят и смена сцены, и сброс
## раунда в меню, и удаление ноды руками: в каждом из этих случаев генерация полосы, если она
## идёт, пишет в буферы, которые сейчас исчезнут вместе с генератором.
...
func _window_target(local_pos: Vector3) -> Vector2i:
	...
	# ШАГ ВОСЬМУШКОЙ ОКНА, А НЕ ЧЕТВЕРТЬЮ. Полоса считается ровно шириной шага на всю глубину: при
	# четверти это 512×2048 — четверть ВСЕЙ начальной генерации, секунды главного потока. Восьмушка
	# вдвое дешевле, а сдвигов всего вдвое больше, и каждый незаметнее.
	var step_x: int = maxi(int(w / 8) / (chunk_size * MACRO_SIZE), 1) * chunk_size * MACRO_SIZE
	var step_z: int = maxi(int(d / 8) / (chunk_size * MACRO_SIZE), 1) * chunk_size * MACRO_SIZE
