...
class Hand extends Control:
	...
	# костяшки сложенных пальцев, слева-снизу — большой.
	const FINGER_W: float = 13.0
	const FIST_X: float = 12.0       # центр кулака правее пальца

	func _draw() -> void:
		var skin := Color(0.98, 0.87, 0.73)
		var shade := Color(0.86, 0.72, 0.58)      # костяшки и складка — на полтона темнее
		var edge := Color(0.14, 0.10, 0.08, 0.92)
		...
		# Контур и заливка — два прохода одних и тех же форм: так рука читается и на светлом
		# песке, и на тёмной панели гаража.
		for pass_i in 2:
			var col: Color = edge if pass_i == 0 else skin
			var g: float = 5.0 if pass_i == 0 else 0.0
			# Кулак: два горизонтальных «бруска» со скруглёнными торцами дают скруглённый
			# прямоугольник без возни с вогнутым полигоном.
			_capsule(Vector2(FIST_X - 8.0, 50.0) + off, Vector2(FIST_X + 8.0, 50.0) + off, 38.0 + g, col)
			_capsule(Vector2(FIST_X - 7.0, 62.0) + off, Vector2(FIST_X + 7.0, 62.0) + off, 34.0 + g, col)
			# Большой палец — наружу влево-вниз.
			_capsule(Vector2(FIST_X - 14.0, 58.0) + off, Vector2(FIST_X - 24.0, 68.0) + off, 13.0 + g, col)
			# Указательный: от кончика в (0,0) вниз, по левому краю кулака.
			_capsule(Vector2(0.0, 7.0) + off, Vector2(0.0, 44.0) + off, FINGER_W + g, col)

		# Костяшки сложенных пальцев по правому краю — три дужки, чтобы кулак не был «шаром».
		for i in 3:
			var c := Vector2(FIST_X + 16.0, 42.0 + float(i) * 11.0) + off
			draw_arc(c, 6.0, -PI * 0.55, PI * 0.55, 10, shade, 2.0)
		# Складка между указательным и кулаком.
		draw_line(Vector2(6.0, 40.0) + off, Vector2(11.0, 46.0) + off, shade, 2.0)
