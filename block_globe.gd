...
extends Control
# «Карусель» выбора блока в стройке (по описанию игрока, docs/atom_picker_reference.md).
# Горизонтальный красный ДИСК-руль (в плоскости XZ) — выбор ТИПА. Вокруг него по кругу
# стоят ВЕРТИКАЛЬНЫЕ кольца-категории (по одному на тип), центры едут по ободу диска.
#   • ТИП: горизонтальный драг КРУТИТ диск (_spin) — кольца едут по кругу; переднее
#     (ближнее к камере) встаёт вертикально ЛИЦОМ к игроку = активная категория (1A).
...
const R_DIAL := 1.05                   # радиус диска-руля (по нему едут центры колец)
const R_RING := 0.62                   # радиус кольца категории (по нему едут блоки)
...
# Центр кольца категории на ободе диска под азимутом β (перед = β≈0 → +Z, к камере).
func _ring_center(beta: float) -> Vector3:
	return R_DIAL * Vector3(sin(beta), 0.0, cos(beta))

# Точка кольца категории: центр на ободе + вертикальный круг радиуса R_RING, стоящий
# ЛИЦОМ наружу (нормаль = радиаль). На переднем (β=0) — лицом к камере, круг в XY.
func _ring_point(theta: float, beta: float) -> Vector3:
	var ct := cos(theta)
	# касательная обода t=(cos β,0,−sin β), вертикаль = Y.
	return _ring_center(beta) + R_RING * Vector3(ct * cos(beta), sin(theta), -ct * sin(beta))
...
func _sync_bg_ovals() -> void:
	...
	if _ring_by_ci.has(front):
		ovals.append({"pts": _ring_oval_px(_beta_of(front)),
				"color": Color(CAT_COLORS[CAT_KEYS[front]], 0.7), "w": 1.6})
	_bg.ovals = ovals
	_bg.queue_redraw()
