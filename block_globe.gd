...
const EQUATOR_R_FRAC := 0.447          # радиус экватора в долях SIZE (плоский 2D-круг,
                                       # см. _equator_px); подобрано расчётом — рамка
                                       # чуть шире максимального экрана активного кольца
...
func _equator_px() -> PackedVector2Array:
	# Экватор — «руль»/безель выбора типа: ПЛОСКИЙ 2D-круг в экранных координатах, БЕЗ
	# прогона через _view/перспективу. Раньше он строился как 3D-круг (нормаль = ось-
	# шпиндель X) — но при текущем виде 3/4 спиндель отклонён от оси камеры на ~67°
	# (посчитано), и такой «3D-экватор» рисовался сильно перекошенным овалом вместо
	# ровного кольца-рамки, как в прототипе игрока (там смотрят точно вдоль спинделя).
	# Плоский круг всегда идеален и не зависит от дальнейшей подстройки VIEW_PITCH/YAW.
	var pts := PackedVector2Array()
	var c := Vector2(SIZE * 0.5, SIZE * 0.5)
	var r := SIZE * EQUATOR_R_FRAC
	for i in 49:
