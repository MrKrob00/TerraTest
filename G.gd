...
## только тем, чего игрок не видит. Если стёрли ТЕКУЩИЙ слот, обнуляем и память: иначе первый же
## автосейв запишет старые деньги обратно.
func delete_world(n: int) -> void:
...
## ВЫХОД С ПОЛИГОНА — ЭТО ВОЗВРАТ К НАСТОЯЩЕМУ ПРОГРЕССУ, а не просто снятие флага.
##
## На полигоне деньги, изученное и грейд лежат в памяти СВОИ (см. proving_ground._sandbox_progress),
## а запись на диск закрыта. Но закрыта она ПОКА ФЛАГ ПОДНЯТ: стоит его снять, и любой отложенный
## `mark_progress_dirty` — таймер на секунду — дописал бы полигонные числа в настоящий слот.
## Поэтому флаг снимается только здесь, вместе со сбросом отложенной записи и перечитыванием
## прогресса из файла. Двух копий этого порядка быть не должно.
func leave_proving_ground() -> void:
	if not proving_ground:
		return
	proving_ground = false
	_progress_dirty = false      # всё, что насчитал полигон, остаётся в полигоне
	_load_progress()             # деньги, грейд и изученное — снова из файла слота
...
# ЗДЕСЬ БЫЛ РЫНОК, И ОН НЕ РАБОТАЛ. Скидки и наценки висели на ПРОДАЖЕ материалов, а
# показывала их панель в правом верхнем углу, рядом с картой. Чтобы механикой воспользоваться,
# надо было держать в голове четыре строки таблицы, ехать за нужным металлом и успеть вернуться
# за пятнадцать минут — а решение «что везти» игрок принимает уже у продавца, где никакой
# панели нет. Постоянно занятый угол экрана ради числа, нужного раз в четверть часа.
#
# Скидка обязана жить ТАМ, ГДЕ ЕЮ ПОЛЬЗУЮТСЯ. Игрок открывает склад, видит «−30%» на конкретном
# блоке и решает, брать ли его сейчас: ни таблицы, ни памяти, ни отдельного окна.
#
# ТОЛЬКО СКИДКИ, БЕЗ НАЦЕНОК. Наценка в магазине — это штраф за то, что игрок зашёл не в ту
# минуту: она ничего не открывает и ничего не предлагает, только просит подождать.
const SALE_WINDOW := 900.0          # 15 минут: витрина должна пережить поездку за деньгами
const SALE_COUNT := 3
## Ступени скидки, по одной на позицию распродажи: −20 / −30 / −40 %. Разные, а не одна на всех:
## одинаковая скидка на три блока читается как «магазин просто подешевел», а разная — как выбор.
const SALE_STEPS := [0.8, 0.7, 0.6]

signal shop_sales_changed

var shop_sales: Dictionary = {}     # G.Block (int) → множитель цены (только там, где он есть)
var _sale_left: float = 0.0
...
## Сколько секунд до смены витрины — для UI.
func shop_sale_seconds_left() -> float:
	return maxf(_sale_left, 0.0)

func _roll_shop_sales() -> void:
	_sale_left = SALE_WINDOW
	shop_sales.clear()
	# Скидка ТОЛЬКО на то, что игрок может купить ПРЯМО СЕЙЧАС. Скидка на запертый блок — это
	# потраченная впустую позиция: увидеть её игрок увидит, а воспользоваться не сможет, и
	# распродажа из трёх штук превратилась бы в распродажу из одной.
	var pool: Array = []
	for bt in BLOCK_META:
		if is_block_shop_unlocked(int(bt)):
			pool.append(int(bt))
	pool.shuffle()
	for i in mini(SALE_COUNT, pool.size()):
		shop_sales[pool[i]] = SALE_STEPS[i % SALE_STEPS.size()]
	shop_sales_changed.emit()

## Цена блока С УЧЁТОМ РАСПРОДАЖИ. Только для магазина — и на витрине, и при покупке.
##
## shop_price() при этом остаётся БАЗОВОЙ и нетронутой, и это не мелочь: по ней меряется
## стоимость машины — какого врага прислать (_pick_preset) и сколько ДИ дать за убитого. Пусти
## скидку туда, и вместе с ней поедет весь баланс: купил блок дешевле — и противник стал слабее.
func shop_price_now(bt: int) -> int:
	return maxi(int(round(shop_price(bt) * float(shop_sales.get(bt, 1.0)))), 1)

## Скидка на блок в процентах; 0 — скидки нет. Для витрины.
func shop_sale_pct(bt: int) -> int:
	return int(round((1.0 - float(shop_sales.get(bt, 1.0))) * 100.0))

## Сколько ДАЮТ за материал. С уходом рынка (см. выше) это ровно базовая цена, и функция
## оставлена отдельной намеренно: она — единственная дверь, через которую продавец и скрапер
## спрашивают цену, и вернуть колебания, если они когда-нибудь понадобятся, можно здесь одной
## строкой, ничего больше не трогая.
func sell_price(key: String) -> int:
	return base_price(key)

## Базовая цена вида материала — из неё считается всё остальное.
...
# ── ДАВЛЕНИЕ МИРА ────────────────────────────────────────────────────────────
## СКОЛЬКО МИРА СЕЙЧАС ВКЛЮЧЕНО: 0 в начале игры, 1 — «нормальная» игра. Одно число на всех, кто
## решает, сколько врагов и как часто: поток спавнера, частота событий, сколько их держится в
## журнале и какие вообще открыты.
##
## ВРЕМЯ = СЛОЖНОСТЬ = НАГРАДА, в этом порядке. Игрок, который ещё собирает первую машину, из
## полноценного давления выносит ровно один урок: «меня разбирают». Сложность обязана догонять
## его умение, а не ждать его на старте; награда за это и так растёт сама — награды событий
## расписаны лесенкой (200 → 420), а выкуп за врага считается от его сборки.
##
## СЧИТАЕМ ОТ ГРЕЙДА, А НЕ ОТ ЧАСОВ В СЕЙВЕ. Грейд — это «сколько игрок уже сделал»; часы можно
## простоять в гараже, и мир, разогнавшийся по таймеру, наказал бы как раз того, кто осторожничает.
## Первые три грейда мир держится слабым, к четвёртому выходит на норму.
const THREAT_FULL_GRADE := 4

func threat_ramp() -> float:
	var g: int = grade("start")
	return clampf(float(g - 1) / float(maxi(THREAT_FULL_GRADE - 1, 1)), 0.0, 1.0)

## Значение, растущее вместе с миром: at_start в начале, at_full к THREAT_FULL_GRADE.
func threat_lerp(at_start: float, at_full: float) -> float:
	return lerpf(at_start, at_full, threat_ramp())
