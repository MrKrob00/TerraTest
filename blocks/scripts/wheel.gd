...
## ОСЬ НАД ЗЕМЛЁЙ: радиус шины плюс просадка рычага. Это и есть клиренс, который колесо даёт.
##
## КОЛЛАЙДЕР БЛОКА ОБЯЗАН СОВПАДАТЬ С КОЛЕСОМ. Он был кубом 1x1x1 у всех трёх, и малое колесо
## из-за этого чертило: ось у него встаёт на 0.49 м, а полкуба — это 0.5, то есть девять
## миллиметров в грунте ещё до первой кочки. Размер коробки в сцене идёт по шине (0.6 / 1.0 /
## 1.3), иначе «маленькое колесо = низкая посадка» упирается не в землю, а в собственный габарит.
