...
##   • верхнее крепится СВЕРХУ, а не сбоку. Нужно перевёрнутой машине: она едет на них, и
##     probe_ground у них сработает сам собой — луч всегда идёт вниз В МИРОВЫХ осях, так что
##     «какое колесо сейчас нижнее» решает не блок, а положение машины;
...
## взяты от обычного колеса, геометрия появится вместе с моделью.
