...
## Может ли это колесо вообще достать до земли. У ВЕРХНЕГО колеса ride_height = 0: оно смотрит
## вверх и грунта не касается никогда. Тяги и грузоподъёмности от него быть не может, а в витрине
## гаража оно их давало — rated_power() считал все is_drive подряд, и панель обещала лишние
## ньютоны, которых в мире не появлялось ни разу.
func touches_ground() -> bool:
