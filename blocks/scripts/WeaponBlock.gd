...
## ЧИСЛА ПОДНЯТЫ ПОД ВЕРШИННОЕ ОСВЕЩЕНИЕ, и это не «сделаем поярче». При
## force_vertex_shading (project.godot) точечные источники считаются В ВЕРШИНАХ и
## интерполируются по треугольнику — в шейдере движка цикл по omni стоит внутри вершинного
## блока. Свет радиусом 6 м на кубе из восьми вершин попадал почти никуда, а glow, который
## обычно и продаёт вспышку ореолом, у нас выключен ценой кадров.
##
## Поэтому радиус берём такой, чтобы в него попадало много вершин (у земли на ближнем уровне
## сетка метровая), а яркость — такую, чтобы хватило после интерполяции. Тонемаппинг AgX светами
## не клиппит, а заваливает, так что большое число здесь не выжигает кадр.
##
## Выглядеть это будет мягкой засветкой, а не резким пятном: при освещении по вершинам иначе
## не бывает. Резкость стоила бы отказа от force_vertex_shading, то есть кадров.
var flash_energy: float = 9.0
