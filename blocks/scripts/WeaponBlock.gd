...
## ВО СКОЛЬКО КУПОЛУ ОБХОДИТСЯ ОЧКО УРОНА ЭТОГО ОРУЖИЯ. Переменная, а не константа: подкласс
## ставит своё значение в `_ready`, как `flash_color` и `yaw_limit`, и второй проверки нигде нет.
##
## Щит перестаёт быть одинаковым ответом на любой ствол: под купол надо приходить с тем, что
## его берёт. Правило взято из TerraTech, но по ЕЁ СОБСТВЕННЫМ описаниям, а не по пересказу —
## пересказ переворачивал главное место. Там:
##   • пули (ballistic) — лучшее против щита, попав в купол, они бьют именно его;
##   • лазеры снимают с батареи половину, то есть против купола они плохи — по замерам
##     игроков одна батарея под одним лазером живёт вчетверо дольше, чем под пулемётом;
##   • взрыв проходит СКВОЗЬ купол и достаёт блоки под ним — щит его вообще не держит, и
##     единственная защита от взрыва — радиус побольше, чтобы отодвинуть точку разрыва;
##   • удар и резка (молоты, пилы, буры) со щитом не взаимодействуют вовсе.
##
## У нас всё это уже почти так: BlockFX.explosion бьёт ЗАПРОСОМ СФЕРЫ, а не лучом, поэтому
## разлёт и сейчас проходит сквозь купол и достаёт блоки внутри. Не хватало только цены.
##
## ЕДИНИЦА — ЭТО ПУЛИ, и для них НИЧЕГО НЕ МЕНЯЕТСЯ: SHIELD_COST_X настроен ровно на них, и
## вся таблица «секунд под огнём» остаётся на месте. Остальное дешевле для купола, то есть хуже
## против него. Бур не обнулён, хотя в оригинале резка щит не видит совсем: буром у нас копают,
## и «шит нельзя сковырнуть вообще» оставило бы сборку без единственного оружия.
##
## РАЗРЫВ У НАС УЖЕ, ЧЕМ В ОРИГИНАЛЕ, И ЭТО НАМЕРЕННО. Там пуля к лазеру относятся как 4 к 1, и
## одна батарея под лазером живёт около двадцати секунд против пяти под пулемётом. У нас лазер —
## САМЫЙ СИЛЬНЫЙ ствол по урону (32 против 25 у пулемёта), поэтому прямая четвёрка почти
## съедается его же уроном, а батарея уходит за двадцать с лишним секунд — столько бой не длится.
...
##
## Дробовик и мортира доворачивают снаряд СРАЗУ после выстрела (конус дроби, навес), и искали
## его так: «последний ребёнок узла Ammo, у которого dir не ноль». Это верно ровно один раз - на
## первом выстреле, пока пул пуст и каждая пуля ДОБАВЛЯЕТСЯ в конец. Дальше пули берутся ИЗ ПУЛА
...
## вылетела пуля.
##
## РАНЬШЕ ПРЕДПОЧИТАЛСЯ `DrillBody2`, А ЭТО МЕШ СТВОЛА, А НЕ ДУЛО. Его начало координат лежит в
## середине турели: у дробовика, мортиры и тяжёлой пушки — единственных трёх, у кого этот узел
...
func _on_bullet_body_entered(body: Node3D, source: Area3D) -> void:
	...
	# Купол платит за попадание ЭНЕРГИЕЙ, и цена зависит от типа урона — поэтому множитель едет
	# вместе с уроном. Узнаём купол по `struck`: это его собственный метод, и обычный блок его
	# не имеет, так что лишней проверки на тип узла не нужно.
	if body.has_method("struck"):
		...
		BlockFX.ground_glitch(self, source.global_position, n if n is Vector3 else Vector3.UP)
