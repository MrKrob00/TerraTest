...
#
# ЗАДАЮТСЯ КУБИКОМ (addons/blockfaces, режимы IN и OUT), поэтому `@export_storage`: значение
# лежит в сцене блока, как и раньше, но галочек в инспекторе у него больше нет. Держать рядом
# с кубиком тот же набор галочек значит иметь два органа управления одним числом — и однажды
# они разойдутся; да и «вход слева» на галочке требует помнить, где у блока лево.
@export_storage var input_faces: int = FACE_BACK       ## Стороны, которыми блок ПРИНИМАЕТ ресурс
@export_storage var output_faces: int = FACE_FRONT     ## Стороны, которыми блок ОТДАЁТ ресурс
...
## ХРАНИМЫЙ ЭКСПОРТ, а не просто поле: их настраивают В РЕДАКТОРЕ, кубиком в инспекторе
## (addons/blockfaces), и значение обязано лежать в сцене блока, а не в коде. Своего вида в
## инспекторе у него нет — ключи вида «0,1,-1|3» руками не пишут.
@export_storage var port_defaults: Dictionary = {}
