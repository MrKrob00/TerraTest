...
## Радиус поражения взрыва (мировые единицы). Клетка блока — метр, так что полтора метра это
## «попал в блок и задел соседний», а не «снёс полборта»: прежние 4.5 накрывали шар в девять
...
func _ready() -> void:
	...
	# Купол взрывчатку почти не держит по цене — но и ракете он не мешает: разлёт проходит
	# сквозь него и достаёт блоки внутри (BlockFX.explosion бьёт сферой, а не лучом).
	shield_cost_mult = SHIELD_MULT_EXPLOSIVE
