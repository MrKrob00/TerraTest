...
#
# Берёт из двух источников: МАТЕРИАЛЫ с земли (что лежит в его зоне) и добычу из КОЛЛЕКТОРОВ
# машин, попавших в зону. Дальше отдаёт по ленте (push_item). Коллектор ничего этого не умеет:
# он лишь копит добытое и отдать может только приёмнику.
#
# СВОБОДНЫЕ БЛОКИ приёмник НЕ подбирает — для них будет отдельный блок-упаковщик.
#
...
## Свободный предмет С ЗЕМЛИ. «Свободный» — по G.is_loose_item: незамороженное тело вне
## машины и вне чужого накопителя. Так отсекаются и блоки чужих машин, и предмет в руке
## игрока (он висит под камерой замороженным).
##
## Раньше приёмник этого не умел вовсе, и не по логике, а по маске: зона стояла на 16 (только
## корпуса машин), то есть ни ресурса, ни блока в мире она физически не видела. Потом мешала
## проверка родителя: руда из жилы лежит не в objects, а под узлом стриминга залежей.
func _try_take_world(body: Node3D) -> bool:
	...
	if inventory.size() >= capacity:
		return false
	# ТОЛЬКО МАТЕРИАЛЫ. Свободные БЛОКИ приёмник не берёт намеренно: упаковка блоков в чанки
	# уедет в отдельный блок-упаковщик (см. docs/BLOCKS_TODO.md), и делать её здесь значило бы
	# завести вторую точку с той же ролью, которую потом пришлось бы отсюда выпиливать.
	if not body.has_method("kind_key"):
		return false                          # не материал — не наше дело
	_accept_item(body)
	return true
