...
#
# Получателю ОБЯЗАТЕЛЕН аккумулятор — без него энергию просто некуда положить: ёмкость
# машины складывается из батарей, и у машины без них energy_cap() равен нулю, то есть
# перелив ушёл бы в пустоту. Поэтому проверяем ёмкость, а не наличие блока по типу: так
# правило само распространится на любые будущие накопители.
