...
## ПРАВЫЙ ВЕРХНИЙ УГОЛ — ЭТО СТОПКА, И СЧИТАЕТ ЕЁ ОДИН КОД. Там живут радар, деньги и трекер
## квестов, и каждый когда-то считал своё место сам — так панель рынка и легла под трекер: тот
## знал про радар и деньги, а про появившуюся между ними панель — нет.
##
## Самой панели рынка больше нет: механика переехала в магазин (см. G, «РАСПРОДАЖА В МАГАЗИНЕ»).
## Правило про одну точку счёта осталось — оно и было причиной, по которой её удалось выдернуть
## отсюда, ничего не сломав.
...
func _push_quest_top(radar_on: bool) -> void:
	var y: float = 0.0
	if radar_on:
		y = _money_bottom()           # под всей стопкой правого верхнего угла
...
## КАК ДАЛЕКО ВИДЕН ЗНАЧОК МАШИНЫ. Значок — это «пересесть В ЭТУ», и висит он над той машиной, к
## которой относится. На шестидесяти метрах он висел над каждой машиной в поле зрения и читался
## как метка на карте, а не как кнопка под рукой.
##
## ПЛАТА ЗА ПЯТНАДЦАТЬ: до базы, оставленной у жилы или у квестовой площадки, придётся подъехать.
## Ровно из-за этого здесь когда-то стояло 14 и было поднято до 60 — если пересаживаться издали
## снова понадобится, это одно число.
const VBTN_SHOW_DIST := 15.0
...
func _on_tech_ui_visibility() -> void:
	...
	# Плавающие панели HUD убираются на время гаража все разом. Группа тут ВТОРАЯ не от лени:
	# «quests» — это конкретное окно с конкретным содержимым, а «hud_float» — любая панель,
	# которая просто висит поверх мира (сейчас — панель полигона). Складывать их в одну группу
	# значило бы, что радар однажды поедет двигать панель разработчика (set_top_offset).
	for q in get_tree().get_nodes_in_group("quests") + get_tree().get_nodes_in_group("hud_float"):
