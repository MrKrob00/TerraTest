...
func _build_ark_drawer() -> void:
	...
	vb.add_child(_make_drawer_button("Музыка", _toggle_music_panel))
...
# ── Панель «Музыка» (открывается из шторки) ────────────────────────────────────
# Три секции (Меню/Путешествие/Бой), строка трека: ♥ любимое (играет чаще) /
# ✖ не играть + «Название — Автор», ▶ у играющего. Сверху: сейчас играет, пропуск,
# громкость. Данные и логика — в autoload Music (music_manager.gd).
var _music_panel: PanelContainer = null
var _music_list: VBoxContainer = null
var _music_now: Label = null
...
func _toggle_music_panel() -> void:
	_set_drawer(false)
	if _music_panel != null and is_instance_valid(_music_panel):
		_music_panel.visible = not _music_panel.visible
		if _music_panel.visible:
			_rebuild_music_list()
		return
	var screen: Vector2 = get_viewport().get_visible_rect().size
	_music_panel = PanelContainer.new()
	_music_panel.add_theme_stylebox_override("panel", _make_panel_style())
	_music_panel.custom_minimum_size = Vector2(430, minf(screen.y * 0.8, 560.0))
	_music_panel.position = (screen - Vector2(430, minf(screen.y * 0.8, 560.0))) * 0.5
	add_child(_music_panel)
	var vb := VBoxContainer.new()
	vb.add_theme_constant_override("separation", 6)
	_music_panel.add_child(vb)
	...
	title.add_theme_color_override("font_color", Color(0.55, 0.85, 0.9))
	title.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	header.add_child(title)
	var skip := Button.new()
	skip.text = "⏭"
	skip.tooltip_text = "Следующий трек"
	skip.custom_minimum_size = Vector2(44, 40)
	...
	close.pressed.connect(func() -> void: _music_panel.visible = false)
	header.add_child(close)

	_music_now = Label.new()
	_music_now.add_theme_font_size_override("font_size", 13)
	_music_now.add_theme_color_override("font_color", Color(0.75, 0.95, 0.8))
	vb.add_child(_music_now)
	...
	var m0 := _music()
	vol.value = m0.volume if m0 else 0.8
	...
	_music_list.add_theme_constant_override("separation", 4)
	scroll.add_child(_music_list)

	var m := _music()
	if m:
		m.prefs_changed.connect(_rebuild_music_list)
		m.track_changed.connect(func(_t: String, _a: String) -> void:
			if _music_panel and _music_panel.visible:
				_rebuild_music_list())
	_rebuild_music_list()

func _rebuild_music_list() -> void:
	if _music_list == null or not is_instance_valid(_music_list):
		return
	...
	_music_now.text = ("▶ %s — %s  [%s]" % [cur.get("title", ""), cur.get("author", ""), m.context_name()]) \
	...
	for s in sections:
		...
		lbl.add_theme_color_override("font_color", Color(0.55, 0.75, 0.8, 0.85))
		_music_list.add_child(lbl)
		var list: Array = m.tracks.get(s[1], [])
		if list.is_empty():
			var empty := Label.new()
			empty.text = "   (треков пока нет — кинь .ogg в music/)"
			empty.add_theme_font_size_override("font_size", 12)
			empty.modulate = Color(1, 1, 1, 0.45)
			_music_list.add_child(empty)
			continue
		for t in list:
			_music_list.add_child(_music_row(m, t, cur))
