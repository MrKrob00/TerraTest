...
##
## Но цвет обязан остаться КРАСНЫМ. Голубой читался как «своё» — в игре, где у игрока бывает
## несколько машин, холодный цвет над машиной означает союзника, и подпись сбивала с толку
## ровно там, где по ней надо стрелять. Поэтому квестовый враг — это яркая, СВЕТЛАЯ красная
## метка: тот же смысл, что у обычной, но заметнее.
const COL_QUEST := Color(1.0, 0.55, 0.45)
...
# ── "It noticed you" ──────────────────────────────────────────────────────────
# Raised by enemy_vehicle._alarm (the setter on its target, the single door every way of picking a
# target goes through). The sign is ASSEMBLED FROM THE GLITCH CARDS a block or a machine appears
# with (BlockFX.glyph): they come up scattered over the diamond, fly together and lock into one "!".
# The colour is the glitch's own; the shape is the whole message.
const BANG := ["##", "##", "##", "##", "##", "..", "##"]
...
func alert() -> void:
	BlockFX.glyph(self, BANG, BANG_CELL, BANG_AT, ALERT_TIME)
