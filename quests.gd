...
func _ready() -> void:
	# НА ПОЛИГОНЕ ЗАДАНИЙ НЕТ, И ПОКАЗЫВАТЬ ИХ НЕЧЕСТНО. Ветки там не идут (quest_arcs молчит),
	# значит ни один участник, посылка или цель в мире не появится — а трекер всё равно рисовал
	# бы шаг и звал убить разведчика, которого никто не пришлёт. Прячем весь слой: Q при этом
	# живёт как жил, его состояние всё равно не попадёт на диск (G._flush_progress).
	if G.proving_ground:
		visible = false
		set_process(false)
